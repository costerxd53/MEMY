...
while(imy < l)
{
  document.getElementById('mems').innerHTML += '<div ontouchstart="gen(1)" id="m'+imy+'"><div class="gallery-module__slide"><img src="loading.gif" style="width: '+screen.width+'px" ></div></div>';
  imy++;
}
}
gen(200);
...
function memget(numerr) {
  ...
  xmlhttp.onreadystatechange=function() {
    if (this.readyState==4 && this.status==200) {
      splitt(this.responseText);
    }
    else if(this.readyState!=4) {
      //łączenie / błąd
    }
  }
  xmlhttp.open("GET","http://77.252.137.224/1e1a367eaf7ceb145406793db4308f10/index.php?numer="+numerr,true);
  xmlhttp.send();
}
function splitt(str) {
  ...
  link = parts[0];
  numer = parts[1];
  document.getElementById('m'+idmem).innerHTML = '<div class="gallery-module__slide"><img src="'+link+'" style="width: '+screen.width+'px" ></div> ';
  idmem++;
  memget(numer);
}
...
(function (window, document, undefined) {
     ...
     */
     function calculateAspectRatio(width, height, ar) {
         var ar = ar ? ar : screen.width+':'+screen.height,
             arValues = ar.split(':'),
